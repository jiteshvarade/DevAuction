...
export default function ProfileHero({resp,Data,showFollow,setShowFollow,showFollowing,setShowFollowing,
                showMenu,setShowMenu,showEdit,setShowEdit,searchprof = false,}) 
{
  const { user } = useAuth0(); 
  ...
  const followMe = async () => {
    const res = await fetch("http://in1.localto.net:5947/profile/follow", {
      method: "POST",
      body: JSON.stringify({ from:user.email , to: Data.userData.UserInfo.email }),
      headers: {
        ...
        <div className="right flex items-center gap-2 pr-4">
          ...
          <div className="profilePic aspect-square md:w-12 w-8  bg-white rounded-full overflow-hidden">
            <img
              ...
              alt={
                user ? `${user.name}'s profile image` : "Default profile image"
              }
